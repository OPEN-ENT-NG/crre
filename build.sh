...
buildNode () {
  case `uname -s` in
    MINGW*)
      docker-compose run --rm -u "$USER_UID:$GROUP_GID" node sh -c "yarn install --no-bin-links --production=false && node_modules/gulp/bin/gulp.js build && yarn run build:sass"
      ;;
    *)
