...
public class DefaultWorkflowService implements WorkflowService {
    ...
    @Override
    public Future<Map<String, List<String>>> getWorkflowListFromStructureScope(String userId, List<String> structureIdList) {
                ...
                promise.complete(event.right().getValue().stream()
                        ...
                        .collect(Collectors.toMap(Map.Entry::getKey,
                                stringListEntry -> stringListEntry.getValue().stream()
                                        .map(jsonObject -> jsonObject.getString("w.name"))
