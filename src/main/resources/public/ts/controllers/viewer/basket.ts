...
export const basketController = ng.controller('basketController',
    ['$scope', '$routeParams', ($scope, $routeParams) => {
        ...
        $scope.calculateNumberOfEquipments = (baskets: Baskets) => {
            let numberOfEquipments = 0;
            baskets.all.map((basket) => {
        ...
        $scope.takeClientOrder = async (basket_name: string) => {
            ...
            totalPriceOrder = parseFloat(totalPriceOrder).toLocaleString(undefined,{minimumFractionDigits: 2, maximumFractionDigits: 2});
            let nbrEquipment = $scope.calculateNumberOfEquipments($scope.baskets_test);
            let {status, data} = await $scope.baskets_test.takeOrder(parseInt($routeParams.idCampaign), $scope.current.structure, basket_name);
            if(status === 200) {
                $scope.confirmOrder(data, nbrEquipment, totalPriceOrder)
                await $scope.baskets_test.sync(parseInt($routeParams.idCampaign), $scope.current.structure.id);
                $scope.cancelConfirmBasketName();
            }
        };

        $scope.confirmOrder = (data, nbr_equipment, totalPriceOrder) => {
